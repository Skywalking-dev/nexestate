...
export async function POST(): Promise<NextResponse> {
	...
	if (connError) {
		...
		return NextResponse.json(
			{ error: "Error al obtener la conexión CRM." },
			{ status: 500 },
		);
	}
	if (!connection) {
		return NextResponse.json(
			{ error: "No hay conexión CRM activa para esta organización." },
			{ status: 404 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{ error: "Error interno al procesar credenciales." },
			{ status: 500 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{ error: "Error al obtener propiedades de Tokko." },
			{ status: 502 },
		);
	}
	...
	const rows = allProperties.map((p) => {
		...
		return {
			...
			surface: p.surface ? Number.parseFloat(p.surface) || null : null,
	...
	if (upsertError) {
		...
		return NextResponse.json(
			{ error: "Error al guardar propiedades." },
			{ status: 500 },
		);
	}
	...
	if (syncUpdateError) {
		console.error(
			"[properties/sync] last_sync_at update error",
			syncUpdateError,
		);
